...
-- Add a UNIQUE constraint to prevent duplicates (without failing if it already exists)
SET @idx_exists := (
  SELECT COUNT(1)
  FROM information_schema.statistics
  WHERE table_schema = DATABASE()
    AND table_name = 'playerbots_rpg_races'
    AND index_name = 'uk_playerbots_rpg_races_entry_race_levels'
);

SET @sql := IF(@idx_exists = 0,
  'ALTER TABLE `playerbots_rpg_races` ADD UNIQUE KEY `uk_playerbots_rpg_races_entry_race_levels` (`entry`,`race`,`minl`,`maxl`);',
  'SELECT 1;'
);
...
-- Seed data without wiping: INSERT IGNORE prevents duplicates thanks to the UNIQUE key above
INSERT IGNORE INTO `playerbots_rpg_races` (`id`,`entry`,`race`,`minl`,`maxl`) VALUES
--
--       DRAENEI
--
