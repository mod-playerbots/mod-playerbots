...
void Engine::LogAction(char const* format, ...)
{
    ...
    if (sPlayerbotAIConfig.logInGroupOnly && (!bot->GetGroup() || !(IsRealPlayer(botAI->GetMaster()) || IsSelfBot(botAI->GetMaster()))) && !testMode)
        return;
...
void Engine::LogValues()
{
    ...
    Player* bot = botAI->GetBot();
    if (sPlayerbotAIConfig.logInGroupOnly && (!bot->GetGroup() || !(IsRealPlayer(botAI->GetMaster()) || IsSelfBot(botAI->GetMaster()))))
        return;
