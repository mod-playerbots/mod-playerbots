...
std::string const Qualified::MultiQualify(std::vector<std::string> const& qualifiers, std::string const& separator, std::string_view const brackets)
{
    std::stringstream out;
    for (uint8 i = 0; i < qualifiers.size(); ++i)
    {
        std::string const& qualifier = qualifiers[i];
