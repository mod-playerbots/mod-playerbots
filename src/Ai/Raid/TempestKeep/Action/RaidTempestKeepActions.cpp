...
bool AlarAssistTanksPickUpEmbersAction::HandlePhase2Embers(Unit*)
{
...
{
    return botAI->DoSpecificAction(CreateNextAction<EquipUpgradeAction>(1.0f).factory, Event(), true);
}
...
bool KaelthasSunstriderLootLegendaryWeaponsAction::Execute(Event /*event*/)
{
    ...
    for (auto const& weapon : weapons)
    {
        if (ShouldBotLootWeapon(weapon.npcEntry))
        {
            ...
            {
                std::unique_ptr<Action> instantiatedAction = CreateAction<EquipAction>(this->botAI);
                EquipAction* equipAction = dynamic_cast<EquipAction*>(instantiatedAction.get());
