...
{
    Value<Unit*>* const findTargetValue = context->GetValue<Unit*>("find target", "leotheras the blind");
    ...
    const Unit* const leotheras = findTargetValue->Get();


    if (leotheras == nullptr)
    {
        return false;
    }

    const Map* const map = leotheras->GetMap();
    ...
    const uint32_t instanceId = map->GetInstanceId();
    ...
    // Human Phase
    const Unit* const leotherasHuman = GetLeotherasHuman(this->botAI);
    const Unit* const leotherasPhase3Demon = GetPhase3LeotherasDemon(this->botAI);

    if (leotherasHuman != nullptr && leotherasPhase3Demon == nullptr)
    {
    ...
    const Unit* const leotherasPhase2Demon = GetPhase2LeotherasDemon(this->botAI);

    // Demon Phase
    if (leotherasPhase2Demon != nullptr)
    {
...
bool MorogrimTidewalkerMoveBossToTankPositionAction::MoveToPhase1TankPosition(Unit*)
{
...
bool MorogrimTidewalkerMoveBossToTankPositionAction::MoveToPhase2TankPosition(Unit*)
{
...
bool LadyVashjAssignPhase2AndPhase3DpsPriorityAction::Execute(Event /*event*/)
{
    ...
    if (bot->GetExactDist2d(center.GetPositionX(), center.GetPositionY()) > 55.0f)
    {
        ...
        Value<Unit*>* const taintedElement = this->context->GetValue<Unit*>("find target", "tainted elemental");
        ...
        if (taintedElement->Get() != nullptr)
        {
            if (
                (designatedLooter && designatedLooter == bot)
...
bool LadyVashjLootTaintedCoreAction::Execute(Event)
{
    ...
    for (auto const& guid : corpses)
    {
        ...
        botAI->AddTimedEvent([botGuid, corpseGuid, vashj]()
        {
...
bool LadyVashjPassTheTaintedCoreAction::LineUpFirstCorePasser(Player* designatedLooter, Unit*)
{
...
bool LadyVashjPassTheTaintedCoreAction::IsFirstCorePasserInIntendedPosition(Player*, Player* firstCorePasser, Unit*)
{
...
bool LadyVashjPassTheTaintedCoreAction::IsSecondCorePasserInIntendedPosition(Player*, Player* secondCorePasser, Unit*)
{
...
bool LadyVashjPassTheTaintedCoreAction::IsThirdCorePasserInIntendedPosition(Player*, Player* thirdCorePasser, Unit*)
{
...
bool LadyVashjPassTheTaintedCoreAction::IsFourthCorePasserInIntendedPosition(Player*, Player* fourthCorePasser, Unit*)
{
