...
float constexpr kTwinWarlockOpeningPullRange = 14.0f;
float constexpr kTwinMeleeContactRange = 5.0f;
float constexpr kTwinAnchorTolerance = 4.0f;
float constexpr kTwinAnchorToleranceEpsilon = 0.25f;
...
float constexpr kTwinMeleeBugServiceRange = 10.0f;
float constexpr kTwinMeleeMarkedBugServiceRange = 12.0f;
...
Unit* GetTwinAttackTarget(Player* bot, PlayerbotAI* botAI)
{
    if (!bot || !botAI)
        return nullptr;
...
bool IsTwinGenericFollowUseful(PlayerbotAI* botAI)
{
    if (!botAI)
        return false;
...
DynamicObject* GetDynamicObjectOwnerIfValid(Aura* aura)
{
    if (!aura || aura->IsRemoved() || aura->IsExpired() || aura->GetType() != DYNOBJ_AURA_TYPE)
        return nullptr;

    DynamicObject* dynObj = aura->GetDynobjOwner();
    return dynObj && dynObj->IsInWorld() ? dynObj : nullptr;
}
...
std::string BuildTwinApproachCleanupReason(Player* bot, PlayerbotAI* botAI)
{
    ...
    if (!Aq40TwinEncounter::HasTwinLocalCleanupState(bot))
        appendToken("unseeded_cleanup");

    if (HasTwinLocalCombatStateToClear(bot, botAI))
        appendToken("stale_combat_state");

    if (Pet* pet = bot ? bot->GetPet() : nullptr)
    {
        if (NeedsTwinPetPassiveCleanup(pet))
            appendToken("pet_cleanup");
    }
    ...
    {
        appendToken("warlock_overlay_cleanup");
    }

    return appended ? reason.str() : std::string("cleanup_only");
}

void AppendTwinBotTraceFields(std::ostringstream& fields, Player* bot, PlayerbotAI* botAI,
                              char const* movementReason, bool includeFollowDiagnostics = false)
{
    if (!bot)
        return;
    ...
    fields << " movement_reason=" << (movementReason ? movementReason : "none")
    ...
    if (!includeFollowDiagnostics || !botAI)
        return;

    Player* const followLeader = GetTwinFollowLeader(botAI);
    fields << " follow_target=" << Aq40Helpers::GetAq40LogUnit(followLeader)
           << " follow_strategy_nc=" << (botAI->HasStrategy("follow", BOT_STATE_NON_COMBAT) ? 1 : 0)
           << " follow_strategy_combat=" << (botAI->HasStrategy("follow", BOT_STATE_COMBAT) ? 1 : 0)
           << " follow_useful=" << (IsTwinGenericFollowUseful(botAI) ? 1 : 0)
           << " stay_strategy_nc=" << (botAI->HasStrategy("stay", BOT_STATE_NON_COMBAT) ? 1 : 0)
           << " stay_strategy_combat=" << (botAI->HasStrategy("stay", BOT_STATE_COMBAT) ? 1 : 0);

    if (followLeader)
        fields << " follow_distance=" << bot->GetDistance2d(followLeader);
}

void AppendTwinReserveHoldFields(std::ostringstream& fields, Player* bot,
                                 Aq40TwinEncounter::TwinEncounterState const& state,
                                 Aq40TwinEncounter::TwinBoss boss, char const* prefix)
{
    Aq40TwinEncounter::TwinStableOwnership const& ownership = Aq40TwinEncounter::GetOwnership(state, boss);
    if (ownership.reserveOwner.IsEmpty())
    {
        fields << " " << prefix << "_reserve_hold=none"
               << " " << prefix << "_reserve_member=none"
               << " " << prefix << "_reserve_target=none"
               << " " << prefix << "_reserve_pet_target=none";
        return;
    }

    Player* reserve = FindTwinInstanceMember(bot, ownership.reserveOwner);
    Aq40TwinEncounter::TwinRoleAssignment const* reserveAssignment =
        Aq40TwinEncounter::GetAssignmentForMember(state, ownership.reserveOwner);
    if (!reserve || !reserveAssignment)
    {
        fields << " " << prefix << "_reserve_hold=" << (reserve ? "unassigned" : "missing")
               << " " << prefix << "_reserve_member=" << Aq40Helpers::GetAq40LogUnit(reserve)
               << " " << prefix << "_reserve_target=none"
               << " " << prefix << "_reserve_pet_target=none";
        return;
    }

    PlayerbotAI* reserveAI = GET_PLAYERBOT_AI(reserve);
    Unit* const reserveTarget = reserveAI ? GetTwinAttackTarget(reserve, reserveAI) : reserve->GetVictim();
    Unit* const reserveSelectionTarget = reserveAI ? GetTwinSelectionTarget(reserve, reserveAI) : nullptr;
    Unit* const reservePetTarget = GetTwinPetTarget(reserve);
    TwinPrePullAnchorChoice const holdAnchorChoice = GetTwinStableAnchorChoice(state, *reserveAssignment);
    float const anchorError = reserve->GetExactDist2d(
        holdAnchorChoice.anchor.position.GetPositionX(), holdAnchorChoice.anchor.position.GetPositionY());

    std::string holdState = "moving";
    if (Aq40TwinEncounter::HasLockedPickupAnchor(reserve, boss))
    {
        holdState = "locked_pickup";
    }
    else if (Aq40TwinEncounter::IsPrimaryController(state, boss, reserve->GetGUID()))
    {
        holdState = "controller";
    }
    else if (IsTwinVeklorTarget(reserveTarget))
    {
        holdState = "targeting_veklor";
    }
    else if (IsTwinVeknilashTarget(reserveTarget))
    {
        holdState = "targeting_veknilash";
    }
    else if (reserveTarget && Aq40SpellIds::IsTwinBugEntry(reserveTarget->GetEntry()))
    {
        holdState = "targeting_bug";
    }
    else if (ShouldHoldTwinReserveTankAssignmentNow(state, *reserveAssignment))
    {
        holdState = anchorError <= kTwinAnchorTolerance ? "parked" : "off_anchor";
    }
    else
    {
        holdState = anchorError <= kTwinAnchorTolerance ? "staged" : "moving";
    }

    fields << " " << prefix << "_reserve_hold=" << holdState
           << " " << prefix << "_reserve_member=" << Aq40Helpers::GetAq40LogUnit(reserve)
           << " " << prefix << "_reserve_side=" << Aq40TwinEncounter::ToString(reserveAssignment->stableSide)
           << " " << prefix << "_reserve_anchor=" << holdAnchorChoice.label
           << " " << prefix << "_reserve_anchor_error=" << anchorError
           << " " << prefix << "_reserve_target=" << Aq40Helpers::GetAq40LogUnit(reserveTarget)
           << " " << prefix << "_reserve_selection_target=" << Aq40Helpers::GetAq40LogUnit(reserveSelectionTarget)
           << " " << prefix << "_reserve_pet_target=" << Aq40Helpers::GetAq40LogUnit(reservePetTarget);
}
                                      ...
                                      uint32 nowMs = 0)
{
    auto appendBossFields = [&](Aq40TwinEncounter::TwinBoss boss, char const* prefix)
    {
        ...
        AppendTwinReserveHoldFields(fields, bot, state, boss, prefix);
    };
...
std::string FormatTwinMissingCriticalPrePullRoles(uint8 criticalMask)
{
    if ((criticalMask & kTwinCriticalPrePullRequiredMask) == kTwinCriticalPrePullRequiredMask)
        return "none";
...
Aq40TwinEncounter::TwinBoss GetTwinBossForManualTarget(Unit const* target)
{
    return IsTwinVeklorTarget(target) ? Aq40TwinEncounter::TwinBoss::Veklor
                                      : Aq40TwinEncounter::TwinBoss::Veknilash;
}

bool IsTwinManualTankBossOrder(Aq40TwinEncounter::TwinRoleAssignment const& assignment, Unit const* target)
{
    if (!IsTwinTankAssignment(assignment) || !IsTwinEmperorTarget(target))
        return false;
...
bool SeedTwinManualTankOrder(Player* bot, Aq40TwinEncounter::TwinEncounterState& state,
                             Aq40TwinEncounter::TwinRoleAssignment const& assignment,
                             TwinPrePullAnchorChoice const& anchorChoice, Unit* manualTarget,
                             char const* stage)
{
    if (!bot || !manualTarget || !IsTwinManualTankBossOrder(assignment, manualTarget))
        return false;

    Aq40TwinEncounter::TwinBoss const targetBoss = GetTwinBossForManualTarget(manualTarget);
    bool const changed = Aq40TwinEncounter::SetCandidateOwner(state, targetBoss, bot->GetGUID());
    LogTwinTankAttackIntentDecision(bot, state, assignment, anchorChoice, manualTarget, manualTarget,
        stage, "manual_tank_order");
    ...
    fields << "boss=twin phase=" << Aq40TwinEncounter::ToString(state.phase)
           ...
           << " manual_tank_order=1"
           << " target_boss=" << Aq40TwinEncounter::ToString(targetBoss)
           << " target=" << Aq40Helpers::GetAq40LogUnit(manualTarget)
           << " cohort=" << Aq40TwinEncounter::ToString(assignment.cohort)
           << " side=" << Aq40TwinEncounter::ToString(assignment.stableSide)
           << " slot=" << static_cast<uint32>(assignment.slotIndex)
           << " candidate_changed=" << (changed ? 1 : 0)
           << " center_committed=" << state.centerCommittedMemberCount
           << " strict_ready=" << state.strictReadyMemberCount
           << " assigned=" << state.assignments.size();
    AppendTwinAnchorLogFields(fields, bot, anchorChoice.anchor, anchorChoice.label);
    AppendTwinBotTraceFields(fields, bot, GET_PLAYERBOT_AI(bot), "manual_tank_order",
        state.phase == Aq40TwinEncounter::TwinEncounterPhase::PrePull);
    AppendTwinOpeningOwnershipFields(fields, bot, state);
    Aq40Helpers::LogAq40Info(bot, "twin_prepull", "twin:manual_tank_order", fields.str(), 1000);
    return changed;
}

bool TryArmTwinManualTankOrderFromPrePull(Player* bot, Aq40TwinEncounter::TwinEncounterState& state,
                                          Aq40TwinEncounter::TwinRoleAssignment const& assignment,
                                          TwinPrePullAnchorChoice const& anchorChoice, Unit* manualTarget,
                                          char const* stage)
{
    if (!bot || state.phase != Aq40TwinEncounter::TwinEncounterPhase::PrePull ||
        ...
        !IsTwinManualTankBossOrder(assignment, manualTarget))
    {
        return false;
    }

    SeedTwinManualTankOrder(bot, state, assignment, anchorChoice, manualTarget, stage);
    ...
    TwinOpeningTargets const openingTargets = FindTwinOpeningTargetsForPrePull(GET_PLAYERBOT_AI(bot));
    std::ostringstream fields;
    fields << "boss=twin phase=" << Aq40TwinEncounter::ToString(state.phase)
           << " mode=" << Aq40TwinEncounter::ToString(state.mode)
           << " stage=" << (stage ? stage : "unknown")
           << " manual_tank_order=1"
           << " degraded_activation=1"
           << " reason=manual_tank_order"
           << " target=" << Aq40Helpers::GetAq40LogUnit(manualTarget)
           << " target_boss=" << Aq40TwinEncounter::ToString(GetTwinBossForManualTarget(manualTarget))
           << " cohort=" << Aq40TwinEncounter::ToString(assignment.cohort)
           << " side=" << Aq40TwinEncounter::ToString(assignment.stableSide)
           << " slot=" << static_cast<uint32>(assignment.slotIndex)
           << " approach=" << state.approachMemberCount
    ...
    Aq40Helpers::LogAq40Warn(bot, "twin_dual_pull", "twin:manual_tank_order:activation",
        fields.str(), 1000);
                                      ...
                                      char const* reason)
{
    ...
    fields << "boss=twin phase=" << Aq40TwinEncounter::ToString(state.phase)
           ...
           << " selection_target=" << Aq40Helpers::GetAq40LogUnit(selectionTarget)
           << " victim=" << Aq40Helpers::GetAq40LogUnit(bot->GetVictim());
    AppendTwinBotTraceFields(fields, bot, botAI, reason);
    AppendTwinOpeningOwnershipFields(fields, bot, state);
...
bool IsTwinReserveTankLiveControllerWindow(Aq40TwinEncounter::TwinEncounterState const& state,
                                           Aq40TwinEncounter::TwinBoss boss,
                                           Aq40TwinEncounter::TwinRoleAssignment const& assignment)
{
    ...
    Aq40TwinEncounter::TwinStableOwnership const& ownership = Aq40TwinEncounter::GetOwnership(state, boss);
    if (Aq40TwinEncounter::GetPickupOwner(state, boss) == assignment.memberGuid ||
        ownership.stableOwner == assignment.memberGuid ||
        ownership.candidateOwner == assignment.memberGuid)
    {
        return true;
    }

    if (Aq40TwinEncounter::IsSwapPrepActive(state))
        return ownership.reserveOwner == assignment.memberGuid;

    return ownership.expectedOwner == assignment.memberGuid;
}
                                            ...
                                            Aq40TwinEncounter::TwinRoleAssignment const& assignment)
{
    ...
    {
        if (Aq40TwinEncounter::IsSwapPrepActive(state) &&
            Aq40TwinEncounter::GetOwnership(state, Aq40TwinEncounter::TwinBoss::Veklor).reserveOwner ==
                assignment.memberGuid)
        {
            return false;
        }

        if (!IsTwinReserveTankLiveControllerWindow(state, Aq40TwinEncounter::TwinBoss::Veklor, assignment))
            return true;
    ...
    {
        if (Aq40TwinEncounter::IsSwapPrepActive(state) &&
            Aq40TwinEncounter::GetOwnership(state, Aq40TwinEncounter::TwinBoss::Veknilash).reserveOwner ==
                assignment.memberGuid)
        {
            return false;
        }

        if (!IsTwinReserveTankLiveControllerWindow(state, Aq40TwinEncounter::TwinBoss::Veknilash, assignment))
            return true;
                                        ...
                                        GuidVector const& units)
{
    ...
    switch (intent)
    {
        ...
        case TwinTargetIntent::HoldCaster:
                                     ...
                                     Aq40TwinEncounter::TwinRoleAssignment const* assignment)
{
    ...
    bool const trueCasterProfile = IsTwinTrueCasterProfile(bot, botAI);
    if (ShouldSuppressTwinWarlockVeklorThreat(bot, state, assignment))
                             ...
                             TwinBugPriority priority)
{
    switch (GetTwinEffectiveBugServiceCohort(assignment, bot, botAI))
    {
        ...
        case Aq40TwinEncounter::TwinRoleCohort::MeleeDps:
            return priority == TwinBugPriority::Hostile ? kTwinMeleeBugServiceRange
                                                        : kTwinMeleeMarkedBugServiceRange;
                             ...
                             Aq40TwinEncounter::TwinSide serviceSide)
{
    ...
    if (GetTwinEffectiveBugServiceCohort(assignment, bot, botAI) ==
            Aq40TwinEncounter::TwinRoleCohort::MeleeDps &&
        bot->GetDistance2d(bug) > maxServiceDistance + kTwinMeleeContactRange)
    {
        return false;
    }
...
ObjectGuid GetTwinPlannedOwnerGuid(Aq40TwinEncounter::TwinEncounterState const& state,
                                   Aq40TwinEncounter::TwinBoss boss)
{
    Aq40TwinEncounter::TwinStableOwnership const& ownership = Aq40TwinEncounter::GetOwnership(state, boss);
    if (Aq40TwinEncounter::IsSwapPrepActive(state) && !ownership.reserveOwner.IsEmpty())
        return ownership.reserveOwner;

    return ownership.expectedOwner;
}

Aq40TwinEncounter::TwinSide GetTwinPlannedOwnerSide(Aq40TwinEncounter::TwinEncounterState const& state,
                                                    Aq40TwinEncounter::TwinBoss boss)
{
    return GetTwinAssignedSide(state, GetTwinPlannedOwnerGuid(state, boss));
}
                                                        ...
                                                        Aq40TwinEncounter::TwinRoleAssignment const& assignment)
{
    Aq40TwinEncounter::TwinSide const expectedSide =
        Aq40TwinEncounter::IsSwapPrepActive(state)
            ? GetTwinPlannedOwnerSide(state, Aq40TwinEncounter::TwinBoss::Veknilash)
                                       ...
                                       Unit const* target)
{
    ...
    switch (assignment->cohort)
    {
        ...
        case Aq40TwinEncounter::TwinRoleCohort::MeleeTank:
            return IsTwinVeknilashTarget(target) && IsTwinMeleeTankController(state, *assignment);
                                        ...
                                        Aq40TwinEncounter::TwinRoleAssignment const& assignment)
{
    return Aq40TwinEncounter::IsSwapPrepActive(state) &&
           GetTwinPlannedOwnerGuid(state, boss) == assignment.memberGuid;
}
                                    ...
                                    Aq40TwinEncounter::TwinRoleAssignment const& assignment)
{
    ...
    Aq40TwinEncounter::TwinSide const veklorExpectedSide =
        GetTwinPlannedOwnerSide(state, Aq40TwinEncounter::TwinBoss::Veklor);
                                                      ...
                                                      Aq40TwinEncounter::TwinSide side)
{
    ...
    if (side == GetTwinPlannedOwnerSide(state, Aq40TwinEncounter::TwinBoss::Veklor))
    {
                                           ...
                                           Aq40TwinEncounter::TwinRoleAssignment const& assignment)
{
    if (!botAI || !Aq40TwinEncounter::IsSwapPrepActive(state) ||
        assignment.stableSide != GetTwinPlannedOwnerSide(state, Aq40TwinEncounter::TwinBoss::Veklor))
    {
        return nullptr;
    }
                                          ...
                                          Unit const* resolvedTarget = nullptr)
{
    ...
    for (Unit* pinnedTarget : pinnedTargets)
    {
        ...
        if (IsTwinBugTarget(resolvedTarget))
        {
            return ReleaseTwinPinnedTarget(
                bot, botAI, pinnedTarget, state, eventKey, "bug_priority", assignment, intent);
        }
                                 ...
                                 Aq40TwinEncounter::TwinRoleAssignment const& assignment)
{
    ...
    if (bot && botAI)
    {
        ...
        clearedIntent = Aq40TwinEncounter::ClearTwinLocalCombatState(bot, botAI, false) || clearedIntent;
        if (bot->GetTarget())
        {
            bot->SetTarget(ObjectGuid::Empty);
            clearedIntent = true;
    ...
    if (bot->GetExactDist2d(anchor.position.GetPositionX(), anchor.position.GetPositionY()) > kTwinAnchorTolerance)
    {
        ...
        AppendTwinBotTraceFields(fields, bot, botAI, "reserve_hold");
        AppendTwinOpeningOwnershipFields(fields, bot, state);
        ...
        return MoveTwinInside(botAI, bot->GetMapId(), anchor.position.GetPositionX(),
            ...
            MovementPriority::MOVEMENT_COMBAT) || clearedIntent;
    }

    return FaceTwinAnchorIfNeeded(bot, anchor) || clearedIntent;
}
...
Aq40TwinEncounter::TwinAnchor GetTwinMeleeDpsCrossAnchor(Aq40TwinEncounter::TwinSide outgoingSide,
                                                          Aq40TwinEncounter::TwinSide incomingSide,
                                                          uint8 slotIndex)
{
    Aq40TwinEncounter::TwinAnchor const outgoingAnchor =
        GetTwinMeleeDpsStageAnchor(outgoingSide, slotIndex);
    Aq40TwinEncounter::TwinAnchor const incomingAnchor =
        GetTwinMeleeDpsStageAnchor(incomingSide, slotIndex);
    Position const& facingTarget =
        Aq40TwinEncounter::GetGeometry().bossPark[ToSideIndex(incomingSide)].position;
    return MakeMidpointAnchor(outgoingAnchor, incomingAnchor, facingTarget);
}
    ...
    Aq40TwinEncounter::TwinRoleAssignment const& assignment)
{
    ...
    if (Aq40TwinEncounter::IsSwapPrepActive(state) && Aq40TwinEncounter::IsKnownSide(meleeSide))
    {
        Aq40TwinEncounter::TwinSide const outgoingSide = Aq40TwinEncounter::GetOppositeSide(meleeSide);
        if (Aq40TwinEncounter::IsKnownSide(outgoingSide))
            return { GetTwinMeleeDpsCrossAnchor(outgoingSide, meleeSide, slotIndex),
                "veknilash_melee_cross" };
    }

    return { GetTwinMeleeDpsStageAnchor(meleeSide, slotIndex), "veknilash_melee_pack" };
}
...
Aq40TwinEncounter::TwinAnchor GetTwinOpeningVeklorWarlockAnchor(Aq40TwinEncounter::TwinSide side)
{
    Aq40TwinEncounter::TwinEncounterGeometry const& geometry = Aq40TwinEncounter::GetGeometry();
    size_t const sideIndex = Aq40TwinEncounter::IsKnownSide(side) ? ToSideIndex(side) : 0u;
    Aq40TwinEncounter::TwinAnchor const& bossPark = geometry.bossPark[sideIndex];
    return BuildOffsetAnchor(bossPark, geometry.roomCenter.position, kTwinWarlockOpeningPullRange, 0.0f,
        bossPark.position, kTwinWarlockOpeningPullRange);
}
    ...
    switch (assignment.cohort)
    {
        case Aq40TwinEncounter::TwinRoleCohort::WarlockTank:
            if (assignment.stableSide == Aq40TwinEncounter::GetInitialSideForBoss(Aq40TwinEncounter::TwinBoss::Veklor))
                return { GetTwinOpeningVeklorWarlockAnchor(assignment.stableSide), "veklor_opening_cast" };
...
void LogTwinHumanMeleeTankParkDiagnostic(Player* logBot,
                                         Aq40TwinEncounter::TwinEncounterState const& state,
                                         Player* owner, Unit* bossUnit,
                                         char const* refreshReason, bool forceInfoLog)
{
    if (!logBot || !owner || !bossUnit || !IsTwinVeknilashTarget(bossUnit))
        return;

    PlayerbotAI* ownerAI = GET_PLAYERBOT_AI(owner);
    if (ownerAI && !ownerAI->IsRealPlayer())
        return;

    Aq40TwinEncounter::TwinRoleAssignment const* assignment =
        Aq40TwinEncounter::GetAssignmentForMember(state, owner->GetGUID());
    ...
    Aq40TwinEncounter::TwinAnchor const& bossPark = anchorChoice.anchor;
    float const bossParkError =
        bossUnit->GetExactDist2d(bossPark.position.GetPositionX(), bossPark.position.GetPositionY());
    bool const parkWarning = bossParkError > kTwinBossParkWarningError;
    if (!parkWarning && !forceInfoLog)
        return;

    std::ostringstream fields;
    fields << "boss=twin twin_boss=veknilash"
           ...
           << " mode=" << Aq40TwinEncounter::ToString(state.mode)
           << " refresh_reason=" << (refreshReason ? refreshReason : "world")
           << " side=" << Aq40TwinEncounter::ToString(assignment->stableSide)
           << " anchor=" << anchorChoice.label
           << " anchor_x=" << bossPark.position.GetPositionX()
           << " anchor_y=" << bossPark.position.GetPositionY()
           << " anchor_z=" << bossPark.position.GetPositionZ()
           << " boss_park_error=" << bossParkError
           << " warning_threshold=" << kTwinBossParkWarningError
           << " veknilash_pickup="
    ...
    if (parkWarning)
    {
        Aq40Helpers::LogAq40Warn(logBot, "twin_validation",
            "twin:human_tank:veknilash_park_warning", fields.str(), 1000);
        return;
    }

    Aq40Helpers::LogAq40Info(logBot, "twin_validation",
        "twin:human_tank:veknilash_pickup", fields.str(), 1000);
}
                                    ...
                                    char const* reason)
{
    ...
    if (!IsTwinBossWorldOwnerValid(state, boss, ownerGuid))
    {
        ...
        {
            uint32 const nowMs = getMSTime();
            bool const openingGrace =
                state.phase == Aq40TwinEncounter::TwinEncounterPhase::DualPullWindow &&
                !Aq40TwinEncounter::IsPickupEstablished(state, boss) &&
                Aq40TwinEncounter::GetPhaseElapsedMs(state, nowMs) <=
                    kTwinStableControllerConfirmationWindowMs;
            std::ostringstream fields;
            fields << "boss=twin twin_boss=" << Aq40TwinEncounter::ToString(boss)
                   ...
                   << " opening_grace=" << (openingGrace ? 1 : 0);
            std::string const eventKey =
                std::string(openingGrace ? "twin:pickup_world_reject_grace:"
                                         : "twin:pickup_world_reject:") +
                Aq40TwinEncounter::ToString(boss) + ":" + std::to_string(ownerGuid.GetCounter());
            if (openingGrace)
                Aq40Helpers::LogAq40Info(logBot, "twin_validation", eventKey, fields.str(), 1000);
            else
                Aq40Helpers::LogAq40Warn(logBot, "twin_validation", eventKey, fields.str(), 1000);
        }
        return false;
    }
    ...
    if (boss == Aq40TwinEncounter::TwinBoss::Veknilash)
        LogTwinHumanMeleeTankParkDiagnostic(logBot, state, owner, bossUnit, reason, meaningfulChange);
...
bool IsTwinBossTargetForBoss(Unit const* target, Aq40TwinEncounter::TwinBoss boss)
{
    return boss == Aq40TwinEncounter::TwinBoss::Veklor ? IsTwinVeklorTarget(target)
                                                       : IsTwinVeknilashTarget(target);
}

bool IsTwinBossTargetingOwner(Unit* bossUnit, Player* owner, PlayerbotAI* ownerAI)
{
    if (!bossUnit || !owner)
        return false;

    ObjectGuid const ownerGuid = owner->GetGUID();
    if (bossUnit->GetVictim() == owner || bossUnit->GetTarget() == ownerGuid)
        return true;

    return ownerAI && ownerAI->HasAggro(bossUnit);
}

bool HasTwinControllerTargetEvidence(Player* owner, PlayerbotAI* ownerAI,
                                     Aq40TwinEncounter::TwinBoss boss)
{
    if (!owner || !ownerAI)
        return false;

    return IsTwinBossTargetForBoss(GetTwinCurrentTarget(ownerAI), boss) ||
           IsTwinBossTargetForBoss(owner->GetVictim(), boss) ||
           IsTwinBossTargetForBoss(GetTwinSelectionTarget(owner, ownerAI), boss);
}

uint32 GetTwinWarlockOpenerSpellAgeMs(uint32 spellAtMs)
{
    return spellAtMs ? getMSTimeDiff(spellAtMs, getMSTime()) : 0;
}

bool HasRecentTwinWarlockSearingPainEvidence(Aq40TwinEncounter::TwinEncounterState const& state,
                                             ObjectGuid ownerGuid)
{
    return !ownerGuid.IsEmpty() && state.veklorWarlockSearingPainCaster == ownerGuid &&
           state.veklorWarlockSearingPainAtMs &&
           GetTwinWarlockOpenerSpellAgeMs(state.veklorWarlockSearingPainAtMs) <=
               kTwinStableControllerConfirmationWindowMs;
}

bool HasTwinControllerPickupEvidence(Player* owner, PlayerbotAI* ownerAI,
                                     Aq40TwinEncounter::TwinEncounterState const& state,
                                     Aq40TwinEncounter::TwinBoss boss, Unit* bossUnit,
                                     Aq40TwinEncounter::TwinRoleAssignment const* assignment)
{
    if (!owner || !ownerAI || !bossUnit || !assignment || !owner->IsAlive())
        return false;
    ...
    Aq40TwinEncounter::TwinStableOwnership const& ownership = Aq40TwinEncounter::GetOwnership(state, boss);
    bool const designatedOwner =
        Aq40TwinEncounter::GetPickupOwner(state, boss) == ownerGuid ||
        ownership.stableOwner == ownerGuid ||
        ownership.candidateOwner == ownerGuid ||
        ownership.expectedOwner == ownerGuid ||
        Aq40TwinEncounter::IsPrimaryController(state, boss, ownerGuid);
    if (!designatedOwner)
        return false;

    bool const targetEvidence = HasTwinControllerTargetEvidence(owner, ownerAI, boss);
    bool const bossTargetsOwner = IsTwinBossTargetingOwner(bossUnit, owner, ownerAI);
    float const distance = owner->GetDistance2d(bossUnit);
    ...
    if (boss == Aq40TwinEncounter::TwinBoss::Veklor)
    {
        ...
        bool const recentSearingPainEvidence =
            HasRecentTwinWarlockSearingPainEvidence(state, ownerGuid);
        bool const openerEvidence =
            recentSearingPainEvidence ||
            state.veklorWarlockShadowWardCaster == ownerGuid;
        bool const effectiveTargetEvidence = targetEvidence || recentSearingPainEvidence;
        bool const rangeEvidence = distance <= kTwinWarlockMaxRange + 6.0f;
        bool const activeThreatEvidence =
            openerEvidence || bossTargetsOwner || IsTwinVeklorTarget(owner->GetVictim()) ||
            state.phase == Aq40TwinEncounter::TwinEncounterPhase::TeleportWindow ||
            state.phase == Aq40TwinEncounter::TwinEncounterPhase::PickupRecovery;

        return effectiveTargetEvidence && rangeEvidence && activeThreatEvidence;
    }

    bool const rangeEvidence = distance <= 12.0f;
    bool const activeThreatEvidence = bossTargetsOwner || IsTwinVeknilashTarget(owner->GetVictim()) ||
        state.phase == Aq40TwinEncounter::TwinEncounterPhase::TeleportWindow ||
        state.phase == Aq40TwinEncounter::TwinEncounterPhase::PickupRecovery;
    return targetEvidence && (rangeEvidence || activeThreatEvidence);
}

void AppendTwinControllerEvidenceFields(std::ostringstream& fields, Player* owner, PlayerbotAI* ownerAI,
                                        Unit* bossUnit, Aq40TwinEncounter::TwinBoss boss,
                                        Aq40TwinEncounter::TwinEncounterState const& state,
                                        bool evidenceAccepted)
{
    Unit* currentTarget = ownerAI ? GetTwinCurrentTarget(ownerAI) : nullptr;
    Unit* selectionTarget = ownerAI ? GetTwinSelectionTarget(owner, ownerAI) : nullptr;
    ObjectGuid const ownerGuid = owner ? owner->GetGUID() : ObjectGuid::Empty;
    float const distance = owner && bossUnit ? owner->GetDistance2d(bossUnit) : 0.0f;
    bool const targetEvidence = HasTwinControllerTargetEvidence(owner, ownerAI, boss);
    bool const bossTargetsOwner = IsTwinBossTargetingOwner(bossUnit, owner, ownerAI);

    fields << " evidence_accepted=" << (evidenceAccepted ? 1 : 0)
           << " controller_current_target=" << Aq40Helpers::GetAq40LogUnit(currentTarget)
           << " controller_selection_target=" << Aq40Helpers::GetAq40LogUnit(selectionTarget)
           << " controller_victim=" << Aq40Helpers::GetAq40LogUnit(owner ? owner->GetVictim() : nullptr)
           << " boss_victim=" << Aq40Helpers::GetAq40LogUnit(bossUnit ? bossUnit->GetVictim() : nullptr)
           << " boss_targets_controller=" << (bossTargetsOwner ? 1 : 0)
           << " controller_distance=" << distance
           << " target_evidence=" << (targetEvidence ? 1 : 0);

    if (boss == Aq40TwinEncounter::TwinBoss::Veklor)
    {
        bool const recentSearingPainEvidence =
            HasRecentTwinWarlockSearingPainEvidence(state, ownerGuid);
        bool const shadowWardEvidence =
            !ownerGuid.IsEmpty() && state.veklorWarlockShadowWardCaster == ownerGuid &&
            state.veklorWarlockShadowWardAtMs;
        bool const rangeEvidence = distance <= kTwinWarlockMaxRange + 6.0f;
        bool const activeThreatEvidence =
            recentSearingPainEvidence || shadowWardEvidence || bossTargetsOwner ||
            IsTwinVeklorTarget(owner ? owner->GetVictim() : nullptr) ||
            state.phase == Aq40TwinEncounter::TwinEncounterPhase::TeleportWindow ||
            state.phase == Aq40TwinEncounter::TwinEncounterPhase::PickupRecovery;

        fields << " range_evidence=" << (rangeEvidence ? 1 : 0)
               << " active_threat_evidence=" << (activeThreatEvidence ? 1 : 0)
               << " opener_searing_pain=" << (state.veklorWarlockSearingPainAtMs ? 1 : 0)
               << " opener_searing_pain_owner=" << (recentSearingPainEvidence ? 1 : 0)
               << " opener_searing_pain_age_ms="
               << GetTwinWarlockOpenerSpellAgeMs(state.veklorWarlockSearingPainAtMs)
               << " opener_shadow_ward=" << (state.veklorWarlockShadowWardAtMs ? 1 : 0)
               << " opener_shadow_ward_owner=" << (shadowWardEvidence ? 1 : 0)
               << " effective_target_evidence="
               << ((targetEvidence || recentSearingPainEvidence) ? 1 : 0);
        return;
    }

    bool const rangeEvidence = distance <= 12.0f;
    bool const activeThreatEvidence = bossTargetsOwner ||
        IsTwinVeknilashTarget(owner ? owner->GetVictim() : nullptr) ||
        state.phase == Aq40TwinEncounter::TwinEncounterPhase::TeleportWindow ||
        state.phase == Aq40TwinEncounter::TwinEncounterPhase::PickupRecovery;
    fields << " range_evidence=" << (rangeEvidence ? 1 : 0)
           << " active_threat_evidence=" << (activeThreatEvidence ? 1 : 0);
...
void PushTwinControllerCandidate(std::vector<ObjectGuid>& candidates, ObjectGuid guid)
{
    if (guid.IsEmpty() || std::find(candidates.begin(), candidates.end(), guid) != candidates.end())
        return;

    candidates.push_back(guid);
}

std::vector<ObjectGuid> BuildTwinControllerEvidenceCandidates(
    Aq40TwinEncounter::TwinEncounterState const& state, Aq40TwinEncounter::TwinBoss boss)
{
    std::vector<ObjectGuid> candidates;
    Aq40TwinEncounter::TwinStableOwnership const& ownership = Aq40TwinEncounter::GetOwnership(state, boss);
    PushTwinControllerCandidate(candidates, Aq40TwinEncounter::GetPickupOwner(state, boss));
    PushTwinControllerCandidate(candidates, ownership.stableOwner);
    PushTwinControllerCandidate(candidates, ownership.candidateOwner);
    PushTwinControllerCandidate(candidates, ownership.expectedOwner);
    return candidates;
}

bool RefreshTwinBossPickupFromController(Player* logBot,
                                         Aq40TwinEncounter::TwinEncounterState& state,
                                         Aq40TwinEncounter::TwinBoss boss, Unit* bossUnit,
                                         char const* reason)
{
    if (!logBot || !bossUnit || !Aq40TwinEncounter::IsActivePhase(state.phase) ||
        Aq40TwinEncounter::IsTerminalPhase(state.phase))
    {
        return false;
    }

    for (ObjectGuid const& ownerGuid : BuildTwinControllerEvidenceCandidates(state, boss))
    {
        Player* owner = FindTwinInstanceMember(logBot, ownerGuid);
        PlayerbotAI* ownerAI = owner ? GET_PLAYERBOT_AI(owner) : nullptr;
        Aq40TwinEncounter::TwinRoleAssignment const* assignment =
            Aq40TwinEncounter::GetAssignmentForMember(state, ownerGuid);
        bool const evidenceAccepted =
            HasTwinControllerPickupEvidence(owner, ownerAI, state, boss, bossUnit, assignment);

        if (owner)
        {
            std::ostringstream healthFields;
            healthFields << "boss=twin twin_boss=" << Aq40TwinEncounter::ToString(boss)
                         << " owner=" << Aq40Helpers::GetAq40LogUnit(owner)
                         << " source=" << Aq40Helpers::GetAq40LogUnit(bossUnit)
                         << " phase=" << Aq40TwinEncounter::ToString(state.phase)
                         << " refresh_reason=" << (reason ? reason : "controller");
            AppendTwinControllerEvidenceFields(healthFields, owner, ownerAI, bossUnit, boss, state,
                evidenceAccepted);
            AppendTwinOpeningOwnershipFields(healthFields, logBot, state);
            Aq40Helpers::LogAq40Info(logBot, "twin_validation",
                std::string("twin:pickup_controller_health:") + Aq40TwinEncounter::ToString(boss) + ":" +
                    std::to_string(ownerGuid.GetCounter()),
                healthFields.str(), 1000);
        }

        if (!evidenceAccepted)
            continue;

        Aq40TwinEncounter::TwinStableOwnership const ownershipBefore =
            Aq40TwinEncounter::GetOwnership(state, boss);
        bool const pickupWasEstablished = Aq40TwinEncounter::IsPickupEstablished(state, boss);
        ObjectGuid const previousPickupOwner = Aq40TwinEncounter::GetPickupOwner(state, boss);
        uint32 const nowMs = getMSTime();
        ...
        fields << "boss=twin twin_boss=" << Aq40TwinEncounter::ToString(boss)
               ...
               << " phase=" << Aq40TwinEncounter::ToString(state.phase)
               << " refresh_reason=" << (reason ? reason : "controller");
        AppendTwinControllerEvidenceFields(fields, owner, ownerAI, bossUnit, boss, state, true);
        AppendTwinOpeningOwnershipFields(fields, logBot, state);
        Aq40Helpers::LogAq40Info(logBot, "twin_pickup_confirm",
            std::string(Aq40TwinEncounter::ToString(boss)) + ":" + std::to_string(ownerGuid.GetCounter()) +
                ":controller:" + Aq40TwinEncounter::ToString(state.phase),
            fields.str(), 1000);
        return true;
    }
                                 ...
                                 Unit* veklor, Unit* veknilash, char const* reason)
{
    ...
    changed |= RefreshTwinBossPickupFromController(
        logBot, state, Aq40TwinEncounter::TwinBoss::Veklor, veklor, reason);
    changed |= RefreshTwinBossPickupFromController(
        logBot, state, Aq40TwinEncounter::TwinBoss::Veknilash, veknilash, reason);
    return changed;
}
                                ...
                                char const* eventKey, char const* anchorLabel)
{
        ...
        kTwinAnchorTolerance)
    {
        ...
        fields << "boss=twin phase=" << Aq40TwinEncounter::ToString(state.phase)
               ...
               << " slot=" << static_cast<uint32>(assignment.slotIndex);
        AppendTwinAnchorLogFields(fields, bot, anchor, anchorLabel);
        AppendTwinBotTraceFields(fields, bot, botAI, eventKey);
        AppendTwinOpeningOwnershipFields(fields, bot, state);
...
void LogTwinWarlockOpenerAttempt(Player* bot,
                                 Aq40TwinEncounter::TwinEncounterState const& state,
                                 char const* spellName, char const* reason,
                                 char const* result, Unit* target = nullptr)
{
    if (!bot)
        return;

    std::ostringstream fields;
    fields << "boss=twin spell=" << (spellName ? spellName : "unknown")
           ...
           << " target=" << Aq40Helpers::GetAq40LogUnit(target)
           << " distance=" << (target ? bot->GetDistance2d(target) : 0.0f)
           << " current_generic_spell=" << (bot->GetCurrentSpell(CURRENT_GENERIC_SPELL) ? 1 : 0)
           << " current_channeled_spell=" << (bot->GetCurrentSpell(CURRENT_CHANNELED_SPELL) ? 1 : 0)
           << " opener_searing_pain=" << (state.veklorWarlockSearingPainAtMs ? 1 : 0)
           << " opener_shadow_ward=" << (state.veklorWarlockShadowWardAtMs ? 1 : 0);
    Aq40Helpers::LogAq40Info(bot, "twin_validation",
        std::string("twin:warlock_opener:") + (spellName ? spellName : "unknown") + ":" +
                              ...
                              char const* reason)
{
    ...
    {
        LogTwinWarlockOpenerAttempt(bot, state, "shadow_ward", reason, "busy");
        return false;
    }
    ...
    if (alreadyActive)
    {
        ...
        LogTwinWarlockOpenerAttempt(bot, state, "shadow_ward", reason, "already_active");
        return false;
    }
    ...
    if (!botAI->CanCastSpell("shadow ward", bot))
    {
        ...
        LogTwinWarlockOpenerAttempt(bot, state, "shadow_ward", reason, "unavailable_accepted");
        return false;
    }

    if (!botAI->CastSpell("shadow ward", bot))
    {
        LogTwinWarlockOpenerAttempt(bot, state, "shadow_ward", reason, "cast_failed");
        return false;
    }
    ...
    Aq40Helpers::LogAq40Info(bot, "twin_strategy", "twin:warlock_tank:shadow_ward",
        fields.str(), 1000);
    LogTwinWarlockOpenerAttempt(bot, state, "shadow_ward", reason, "cast");
    return true;
}
                               ...
                               Unit* veklor, char const* reason)
{
    ...
    if (bot->GetCurrentSpell(CURRENT_GENERIC_SPELL) || bot->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
    {
        LogTwinWarlockOpenerAttempt(bot, state, "searing_pain", reason, "busy", veklor);
        return false;
    }

    if (!botAI->CanCastSpell("searing pain", veklor))
    {
        LogTwinWarlockOpenerAttempt(bot, state, "searing_pain", reason, "cannot_cast", veklor);
        return false;
    }

    if (!botAI->CastSpell("searing pain", veklor))
    {
        LogTwinWarlockOpenerAttempt(bot, state, "searing_pain", reason, "cast_failed", veklor);
        return false;
    }
    ...
    Aq40Helpers::LogAq40Info(bot, "twin_strategy", "twin:warlock_tank:searing_pain",
        fields.str(), 1000);
    LogTwinWarlockOpenerAttempt(bot, state, "searing_pain", reason, "cast", veklor);
    return true;
}
...
bool TryTwinWarlockThreatSpell(Player* bot, PlayerbotAI* botAI,
                               ...
                               Unit* veklor, char const* reason)
{
    return TryTwinWarlockSearingPain(bot, botAI, state, veklor, reason) ||
           TryTwinWarlockShadowWard(bot, botAI, state, reason);
}

bool IsTwinWarlockInOpeningCastRange(Player const* bot, Unit const* veklor)
{
    return bot && veklor && IsTwinVeklorTarget(veklor) &&
           bot->GetDistance2d(veklor) <= kTwinWarlockMaxRange;
}
                                                        ...
                                                        float tolerance)
{
    ...
    for (GroupReference* ref = group->GetFirstMember(); ref; ref = ref->next())
    {
        ...
        TwinPrePullAnchorChoice const anchorChoice = GetTwinPrePullAnchorChoice(state, *assignment);
        bool ready = member->GetExactDist2d(anchorChoice.anchor.position.GetPositionX(),
                         anchorChoice.anchor.position.GetPositionY()) <= tolerance;

        if (ready && IsTwinOpeningWarlockTankAssignment(*assignment))
        {
            Unit* veklor = FindTwinBossForPrePull(GET_PLAYERBOT_AI(member), Aq40TwinEncounter::TwinBoss::Veklor);
            ready = IsTwinWarlockInOpeningCastRange(member, veklor);
        }
    ...
    evaluation.missingCriticalRoles = FormatTwinMissingCriticalPrePullRoles(evaluation.criticalMask);
    return evaluation;
}
...
void LogTwinPrePullStageWait(Player* bot,
                             ...
                             Unit* target = nullptr,
                             char const* missingCriticalRoles = nullptr)
{
    if (!bot)
        return;
    ...
    fields
           ...
           << " strict_ready=" << state.strictReadyMemberCount
           << " assigned=" << state.assignments.size()
           << " missing_critical_roles=" << (missingCriticalRoles ? missingCriticalRoles : "none")
           << " unsupported_reason=" << (state.unsupportedReason.empty() ? "none" : state.unsupportedReason);
    AppendTwinWarlockPoolTelemetryFields(fields, state);
...
void LogTwinApproachFollowGap(Player* bot, PlayerbotAI* botAI,
                              Aq40TwinEncounter::TwinEncounterState const& state,
                              Aq40TwinEncounter::TwinRoleAssignment const& assignment,
                              TwinPrePullAnchorChoice const& anchorChoice,
                              std::string const& cleanupReason)
{
    if (!bot || !botAI)
        return;

    if (!botAI->HasStrategy("follow", BOT_STATE_NON_COMBAT) || !IsTwinGenericFollowUseful(botAI))
        return;
                               ...
                               TwinPrePullAnchorChoice const& anchorChoice)
{
    ...
    bool const isOpeningWarlock = IsTwinOpeningWarlockTankAssignment(assignment);
    Unit* veklor = isOpeningWarlock ? FindTwinBossForPrePull(botAI, Aq40TwinEncounter::TwinBoss::Veklor)
                                    : nullptr;
    bool const openingRangeReady = !isOpeningWarlock || IsTwinWarlockInOpeningCastRange(bot, veklor);
    if (anchorDistance <= kTwinStrictReadyAnchorTolerance && openingRangeReady)
        return;
    ...
    fields << "boss=twin phase=prepull mode=" << Aq40TwinEncounter::ToString(state.mode)
           ...
           << " veklor_distance=" << (veklor ? bot->GetDistance2d(veklor) : 0.0f)
           << " opening_cast_ready=" << (openingRangeReady ? 1 : 0)
           << " quorum_required=" << Aq40TwinEncounter::GetTwinPrePullQuorumRequirement(state.assignments.size())
           << " approach=" << state.approachMemberCount
           << " staged=" << state.stagedMemberCount
           << " center_committed=" << state.centerCommittedMemberCount
...
bool Aq40TwinApproachStageAction::Execute(Event /*event*/)
{
    ...
    std::string const cleanupReason = BuildTwinApproachCleanupReason(bot, botAI);
    Unit* pendingTankAttackTarget = IsTwinTankAssignment(*assignment) ? GetTwinPendingAttackIntentTarget(bot, botAI)
                                                                      : nullptr;
    TwinPrePullAnchorChoice const anchorChoice = GetTwinPrePullAnchorChoice(*state, *assignment);
    bool const manualTankOrder = IsTwinManualTankBossOrder(*assignment, pendingTankAttackTarget);
    if (manualTankOrder &&
        TryArmTwinManualTankOrderFromPrePull(bot, *state, *assignment, anchorChoice,
            pendingTankAttackTarget, "approach"))
    {
        bool const attackedManualTarget = AttackTwinTarget(botAI, pendingTankAttackTarget);
        bool const petChanged = SyncTwinEncounterPetPolicy(
            bot, botAI, *state, assignment, pendingTankAttackTarget, GuidVector());
        return attackedManualTarget || overlayChanged || petChanged;
    }
    ...
    Aq40TwinEncounter::MarkTwinLocalCleanupState(bot);
    bool const clearedIntent = manualTankOrder ? false : Aq40TwinEncounter::ClearTwinLocalCombatState(bot, botAI, false);
    ...
    if (pendingTankAttackTarget)
    {
        LogTwinTankAttackIntentDecision(bot, *state, *assignment, anchorChoice, pendingTankAttackTarget, nullptr,
            "approach", manualTankOrder ? "manual_tank_order" : "hold_center_commit");
    }
    ...
    return overlayChanged || releasedPinnedBoss || clearedIntent || petChanged || recoveredFollowState;
}
...
bool Aq40TwinPrePullStageAction::Execute(Event /*event*/)
{
    ...
    if (manualTankOrder &&
        TryArmTwinManualTankOrderFromPrePull(bot, *state, *assignment, anchorChoice,
            pendingTankAttackTarget, "prepull"))
    {
        bool const attackedManualTarget = AttackTwinTarget(botAI, pendingTankAttackTarget);
        bool const petChanged = SyncTwinEncounterPetPolicy(
            bot, botAI, *state, assignment, pendingTankAttackTarget, GuidVector());
    ...
    if (distance > kTwinAnchorTolerance)
    {
        ...
        fields
               ...
               << " assigned=" << state->assignments.size()
               << " unsupported_reason=" << (state->unsupportedReason.empty() ? "none" : state->unsupportedReason);
        AppendTwinBotTraceFields(fields, bot, botAI, "distance", true);
        AppendTwinOpeningOwnershipFields(fields, bot, *state);
    ...
    if (!strictReady)
    {
        ...
        LogTwinPrePullStageWait(bot, *state, *assignment, anchorChoice, "strict_ready_pending",
            nullptr, strictReadyEvaluation.missingCriticalRoles.c_str());
...
bool Aq40TwinDualPullEngageAction::Execute(Event /*event*/)
{
    ...
    if (!target)
    {
        if (assignment && assignment->cohort == Aq40TwinEncounter::TwinRoleCohort::RangedDps &&
            intent == TwinTargetIntent::HoldCaster)
        {
            TwinPrePullAnchorChoice const anchorChoice = GetTwinStableAnchorChoice(*state, *assignment);
        ...
        target->GetEntry() == Aq40SpellIds::TwinVeklorNpcEntry)
    {
        ...
        Aq40TwinEncounter::TwinAnchor const& holdAnchor = geometry.stableVeklorWarlock[sideIndex];
        ...
        bool const inOpeningSearingPainRange = distance <= kTwinWarlockMaxRange + 2.0f;
        bool const veklorPickupEstablished =
            Aq40TwinEncounter::IsPickupEstablished(*state, Aq40TwinEncounter::TwinBoss::Veklor);
        float const anchorTolerance = kTwinAnchorTolerance + kTwinAnchorToleranceEpsilon;

        auto const logVeklorOpenerMovement = [&](char const* eventKey, char const* movementReason,
                                                  Aq40TwinEncounter::TwinAnchor const& anchor)
        {
            std::ostringstream fields;
            fields << "boss=twin phase=dual_pull_window side="
                   << Aq40TwinEncounter::ToString(assignment->stableSide)
                   << " reason=" << (movementReason ? movementReason : "veklor_opener")
                   << " target=" << Aq40Helpers::GetAq40LogUnit(target)
                   << " target_distance=" << distance
                   << " in_searing_pain_range=" << (inSearingPainRange ? 1 : 0)
                   << " veklor_pickup=" << (veklorPickupEstablished ? 1 : 0)
                   << " has_threat_lead=" << (hasThreatLead ? 1 : 0);
            AppendTwinAnchorLogFields(fields, bot, anchor, "stable_veklor_warlock");
            AppendTwinBotTraceFields(fields, bot, botAI, movementReason ? movementReason : "veklor_opener");
            AppendTwinOpeningOwnershipFields(fields, bot, *state);
            Aq40Helpers::LogAq40Info(bot, "twin_position", eventKey, fields.str(), 1000);
        };

        bool const targetChanged = AI_VALUE(Unit*, "current target") != target ||
        ...
        if ((!veklorPickupEstablished ? inOpeningSearingPainRange : inSearingPainRange) &&
            TryTwinWarlockSearingPain(bot, botAI, *state, target, "dual_pull_opener_asap"))
        {
            return true;
        }

        if (!veklorPickupEstablished && distance > kTwinWarlockMaxRange + 2.0f)
        {
            Aq40TwinEncounter::RequestImmediateMovementInterrupt(bot);
            logVeklorOpenerMovement("twin:dual_pull:veklor_pickup_stepin",
                "initial_veklor_pickup_range", holdAnchor);
            return MoveNear(target, kTwinWarlockPreferredRange, MovementPriority::MOVEMENT_COMBAT) ||
                   attacked || releasedPinnedBoss || overlayChanged || petSyncChanged || worldPickupChanged;
        }

        if (!veklorPickupEstablished && distance < kTwinWarlockMinRange)
        {
            Aq40TwinEncounter::RequestImmediateMovementInterrupt(bot);
            logVeklorOpenerMovement("twin:dual_pull:veklor_pickup_backstep",
                "initial_veklor_pickup_range", holdAnchor);
        ...
        if (bot->GetExactDist2d(anchor.position.GetPositionX(), anchor.position.GetPositionY()) > anchorTolerance)
        {
            Aq40TwinEncounter::RequestImmediateMovementInterrupt(bot);
            std::ostringstream fields;
            fields << "boss=twin phase=dual_pull_window side="
                   ...
                   << " in_searing_pain_range=" << (inSearingPainRange ? 1 : 0)
                   << " veklor_pickup=" << (veklorPickupEstablished ? 1 : 0)
                   << " anchor_tolerance=" << anchorTolerance;
            AppendTwinAnchorLogFields(fields, bot, anchor, "stable_veklor_warlock");
            AppendTwinBotTraceFields(fields, bot, botAI,
                shouldRotateInward ? "veklor_rotate_inward" : "veklor_stable_hold");
            AppendTwinOpeningOwnershipFields(fields, bot, *state);
            Aq40Helpers::LogAq40Info(bot, "twin_position",
                shouldRotateInward ? "twin:dual_pull:veklor_rotate_inward" : "twin:dual_pull:veklor_hold_cast",
                fields.str(), 1000);
            return MoveInside(bot->GetMapId(), anchor.position.GetPositionX(), anchor.position.GetPositionY(),
                anchor.position.GetPositionZ(), anchorTolerance, MovementPriority::MOVEMENT_COMBAT) ||
                   attacked || releasedPinnedBoss || overlayChanged || petSyncChanged || worldPickupChanged;
        }
        ...
        target->GetEntry() == Aq40SpellIds::TwinVeknilashNpcEntry)
    {
        ...
        if (HasTwinBossAggroLead(bot, botAI, *state, Aq40TwinEncounter::TwinBoss::Veknilash, target))
        {
                ...
                kTwinAnchorTolerance)
            {
                ...
                AppendTwinBotTraceFields(fields, bot, botAI, "veknilash_park");
                AppendTwinOpeningOwnershipFields(fields, bot, *state);
...
bool Aq40TwinSwapPrepStageAction::Execute(Event /*event*/)
{
    ...
    if (armedSwapPrep)
    {
        ...
        fields << "boss=twin phase=" << Aq40TwinEncounter::ToString(state->phase)
               ...
               << " veklor_incoming_side="
               << Aq40TwinEncounter::ToString(GetTwinPlannedOwnerSide(*state, Aq40TwinEncounter::TwinBoss::Veklor))
               << " veknilash_expected_side="
               << Aq40TwinEncounter::ToString(GetTwinExpectedOwnerSide(*state, Aq40TwinEncounter::TwinBoss::Veknilash))
               << " veknilash_incoming_side="
               << Aq40TwinEncounter::ToString(
                      GetTwinPlannedOwnerSide(*state, Aq40TwinEncounter::TwinBoss::Veknilash));
        AppendTwinBotTraceFields(fields, bot, botAI, "swap_prep_arm");
        AppendTwinOpeningOwnershipFields(fields, bot, *state, nowMs);
...
bool Aq40TwinChooseTargetAction::Execute(Event /*event*/)
{
    ...
    bool const meleeDpsVeklorProximityRisk = IsTwinMeleeDpsVeklorProximityRisk(bot, veklor, assignment);
    if (meleeDpsVeklorProximityRisk && assignment)
    {
        bool const releasedForSafety =
            (veknilash ? ReleaseTwinPinnedTarget(bot, botAI, veknilash, *state,
                             "twin:melee_dps:release_veklor_proximity", "veklor_proximity",
                             assignment, TwinTargetIntent::None)
                       : false) ||
            (veklor ? ReleaseTwinPinnedTarget(bot, botAI, veklor, *state,
                          "twin:melee_dps:release_veklor_proximity", "veklor_proximity",
                          assignment, TwinTargetIntent::None)
                    : false);
        TwinPrePullAnchorChoice const anchorChoice = GetTwinStableAnchorChoice(*state, *assignment);
    ...
    Unit* target = ResolveTwinTarget(bot, botAI, *state, assignment, intent, encounterUnits, reason);
    if (target && IsTwinBugTarget(target))
    {
        releasedPinnedBoss = ReleaseTwinPinnedBossTargetForIntent(
                                 bot, botAI, *state, assignment, intent,
                                 "twin:target:release_wrong_target", target) ||
                             releasedPinnedBoss;
    }

    auto const holdAssignedAnchorWithoutTarget = [&]() -> bool
    {
        if (!assignment || Aq40TwinEncounter::HasActiveLockedPickupAnchor(bot))
        {
            return false;
        }
    ...
    if (!target)
        return holdAssignedAnchorWithoutTarget() || releasedPinnedBoss || petChanged || pickupAnchorChanged;
...
bool Aq40TwinDodgeBlizzardAction::Execute(Event /*event*/)
{
    ...
    DynamicObject* blizzardDynObj = GetDynamicObjectOwnerIfValid(blizzardAura);
    bool const hasBlizzardAura = blizzardAura != nullptr;
    bool const hasBlizzardDynObj = blizzardDynObj != nullptr;
...
bool Aq40TwinAvoidVeklorAction::Execute(Event /*event*/)
{
    ...
    bool const meleeDpsVeklorProximityRisk = IsTwinMeleeDpsVeklorProximityRisk(bot, veklor, assignment);
    bool const hunterPetVeklorProximityRisk =
        IsTwinHunterPetVeklorProximityRisk(bot, veklor, assignment);
    if (!meleeDpsVeklorProximityRisk && !hunterPetVeklorProximityRisk &&
        distance > kTwinArcaneBurstDangerRadius && !(arcaneWindow && distance <= kTwinArcaneBurstLooseRadius))
    {
        return false;
    }
    ...
    bool const petChanged = SyncTwinEncounterPetPolicy(bot, botAI, *state, assignment, nullptr, encounterUnits);
    bool const releasedForSafety =
        meleeDpsVeklorProximityRisk && assignment &&
        ((veklor ? ReleaseTwinPinnedTarget(bot, botAI, veklor, *state,
                       "twin:melee_dps:release_veklor_proximity", "veklor_proximity", assignment,
                       TwinTargetIntent::None)
                 : false) ||
            (veknilash ? ReleaseTwinPinnedTarget(bot, botAI, veknilash, *state,
                             "twin:melee_dps:release_veklor_proximity", "veklor_proximity", assignment,
    ...
    return MoveTwinToHazardAnchor(bot, botAI, *state, assignment, anchorChoice,
               "twin:veklor:avoid",
               meleeDpsVeklorProximityRisk ? "melee_veklor_proximity"
               : (hunterPetVeklorProximityRisk ? "pet_veklor_proximity"
                                               : (arcaneWindow ? "arcane_burst_window"
                                                               : "veklor_proximity"))) ||
           releasedForSafety || petChanged;
}
