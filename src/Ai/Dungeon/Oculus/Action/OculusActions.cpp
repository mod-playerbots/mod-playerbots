...
bool AvoidUnstableSphereAction::Execute(Event)
{
...
bool MountDrakeAction::Execute(Event)
{
...
bool DismountDrakeAction::Execute(Event)
{
    if (bot->GetVehicle())
    {
        bot->ExitVehicle();
...
bool OccFlyDrakeAction::Execute(Event)
{
    Player* master = botAI->GetMaster();
    if (!master) { return false; }
...
bool OccDrakeAttackAction::Execute(Event)
{
...
bool AvoidArcaneExplosionAction::Execute(Event)
{
...
bool TimeBombSpreadAction::Execute(Event)
{
    float radius = 10.0f;
