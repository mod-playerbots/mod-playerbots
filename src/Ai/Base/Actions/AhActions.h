...
private:
    bool PostAuctionSell(Item* item, ItemTemplate const* proto,
        ObjectGuid const& auctioneerGuid, uint32 auctioneerFaction,
        PlayerbotAuctionItemPolicy const& policy, uint32 itemCount,
        uint32 unitPrice, PlayerbotAuctionMarketSnapshot const& marketSnapshot);
};
...
private:
    bool ParseAuctionPacket(WorldPacket& p, uint32 gearBudget, uint8 ahBucket, std::vector<AhItem>& buyCandidates);
    AhItem const* PickBestCandidate(std::vector<AhItem> const& buyCandidates);
    bool PostAuctionBid(AhItem const& candidate, ObjectGuid auctioneerGuid, uint8 ahBucket, uint32 gearBudget);
};
