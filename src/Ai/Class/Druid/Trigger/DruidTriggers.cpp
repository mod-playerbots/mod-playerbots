...
{
    return BuffOnPartyTrigger::IsActive() && !botAI->GetServices().GetSpellService().HasAura("gift of the wild", GetTarget());
}

bool MarkOfTheWildTrigger::IsActive()
{
    return BuffTrigger::IsActive() && !botAI->GetServices().GetSpellService().HasAura("gift of the wild", GetTarget());
}

bool ThornsOnPartyTrigger::IsActive()
{
    return BuffOnPartyTrigger::IsActive() && !botAI->GetServices().GetSpellService().HasAura("thorns", GetTarget());
}
...
bool ThornsTrigger::IsActive() { return BuffTrigger::IsActive() && !botAI->GetServices().GetSpellService().HasAura("thorns", GetTarget()); }

bool BearFormTrigger::IsActive() { return !botAI->GetServices().GetSpellService().HasAnyAuraOf(bot, "bear form", "dire bear form", nullptr); }

bool TreeFormTrigger::IsActive() { return !botAI->GetServices().GetSpellService().HasAura(33891, bot); }
