...
void WotlkDungeonDTKStrategy::InitTriggers(std::vector<TriggerNode*> &triggers)
{
    ...
    triggers.push_back(new TriggerNode("corpse explode",
        { new NextAction("corpse explode spread", ACTION_MOVE + 5) }));
    ...
    triggers.push_back(new TriggerNode("arcane field",
        { new NextAction("avoid arcane field", ACTION_MOVE + 5) }));
    triggers.push_back(new TriggerNode("arcane field",
        { new NextAction("novos positioning", ACTION_MOVE + 4) }));
    triggers.push_back(new TriggerNode("arcane field",
        { new NextAction("novos target priority", ACTION_NORMAL + 1) }));
    ...
    triggers.push_back(new TriggerNode("gift of tharon'ja",
        { new NextAction("touch of life", ACTION_NORMAL + 5) }));
    triggers.push_back(new TriggerNode("gift of tharon'ja",
        { new NextAction("bone armor", ACTION_NORMAL + 4) }));
    ...
    triggers.push_back(new TriggerNode("tharon'ja out of melee",
        { new NextAction("reach melee", ACTION_NORMAL + 3) }));
    triggers.push_back(new TriggerNode("gift of tharon'ja",
        { new NextAction("taunt", ACTION_NORMAL + 2) }));
    triggers.push_back(new TriggerNode("gift of tharon'ja",
        { new NextAction("slaying strike", ACTION_NORMAL + 2) }));
}
