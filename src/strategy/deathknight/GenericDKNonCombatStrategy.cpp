...
void GenericDKNonCombatStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(
        new TriggerNode("no pet", { new NextAction("raise dead", ACTION_NORMAL + 1) }));
    triggers.push_back(
        new TriggerNode("horn of winter", { new NextAction("horn of winter", 21.0f) }));
    triggers.push_back(
        new TriggerNode("bone shield", { new NextAction("bone shield", 21.0f) }));
    triggers.push_back(
