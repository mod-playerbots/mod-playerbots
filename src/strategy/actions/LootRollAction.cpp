...
static bool GroupHasPreferredIntApUser(Player* self)
{
    ...
    for (GroupReference* it = g->GetFirstMember(); it; it = it->next())
    {
        ...
        if (!GET_PLAYERBOT_AI(p))
            continue;  // IGNORE humans: only bots are considered
...
// Try to detect the spell taught by a recipe and whether the bot already knows it. If we can’t resolve the taught spell
// reliably, we fall back to "has profession + skill rank OK".
...
// Maps a RECIPE subclass & item metadata to the SkillLine needed (when RequiredSkill is not set). In DBs, recipes
// normally have RequiredSkill filled; we keep this as a fallback.
...
static bool IsWeaponOrShieldOrRelicAllowedForClass(SpecTraits const& traits, ItemTemplate const* proto)
{
    ...
    if (proto->InventoryType == INVTYPE_RELIC)
    {
        ...
        return traits.cls == CLASS_DEATH_KNIGHT || traits.cls == CLASS_DRUID || traits.cls == CLASS_PALADIN ||
               traits.cls == CLASS_SHAMAN;
    }
    ...
    switch (proto->SubClass)
    {
        ...
        case ITEM_SUBCLASS_WEAPON_AXE2:
            ...
            return traits.cls == CLASS_DEATH_KNIGHT || traits.cls == CLASS_HUNTER || traits.cls == CLASS_PALADIN ||
                   traits.cls == CLASS_SHAMAN || traits.cls == CLASS_WARRIOR ||
                   traits.cls == CLASS_ROGUE;  // Rogue: 1H axes

        // Swords
        case ITEM_SUBCLASS_WEAPON_SWORD:  // 1H swords
            return traits.cls == CLASS_DEATH_KNIGHT || traits.cls == CLASS_HUNTER || traits.cls == CLASS_MAGE ||
                   traits.cls == CLASS_PALADIN || traits.cls == CLASS_ROGUE || traits.cls == CLASS_WARRIOR ||
        ...
        case ITEM_SUBCLASS_WEAPON_MACE:  // 1H maces
            return traits.cls == CLASS_DEATH_KNIGHT || traits.cls == CLASS_DRUID || traits.cls == CLASS_PALADIN ||
                   traits.cls == CLASS_PRIEST || traits.cls == CLASS_SHAMAN || traits.cls == CLASS_WARRIOR ||
                   traits.cls == CLASS_ROGUE;  // Rogue: 1H maces in WotLK
        case ITEM_SUBCLASS_WEAPON_MACE2:       // 2H maces
            return traits.cls == CLASS_DEATH_KNIGHT || traits.cls == CLASS_DRUID || traits.cls == CLASS_PALADIN ||
                   traits.cls == CLASS_WARRIOR;  // Shaman: no 2H maces

        // Polearms
        case ITEM_SUBCLASS_WEAPON_POLEARM:
            return traits.cls == CLASS_DEATH_KNIGHT || traits.cls == CLASS_DRUID || traits.cls == CLASS_HUNTER ||
                   traits.cls == CLASS_PALADIN || traits.cls == CLASS_WARRIOR;  // Shaman: cannot use polearms

        // Staves
        case ITEM_SUBCLASS_WEAPON_STAFF:
            return traits.cls == CLASS_DRUID || traits.cls == CLASS_HUNTER || traits.cls == CLASS_MAGE ||
                   traits.cls == CLASS_PRIEST || traits.cls == CLASS_SHAMAN || traits.cls == CLASS_WARLOCK;

        // Daggers
        case ITEM_SUBCLASS_WEAPON_DAGGER:
            return traits.cls == CLASS_DRUID || traits.cls == CLASS_HUNTER || traits.cls == CLASS_MAGE ||
                   traits.cls == CLASS_PRIEST || traits.cls == CLASS_ROGUE || traits.cls == CLASS_WARLOCK ||
                   traits.cls == CLASS_WARRIOR;  // Warriors can use daggers

        // Fist weapons
        case ITEM_SUBCLASS_WEAPON_FIST:
            return traits.cls == CLASS_DRUID || traits.cls == CLASS_HUNTER || traits.cls == CLASS_ROGUE ||
                   traits.cls == CLASS_SHAMAN || traits.cls == CLASS_WARRIOR;
...
static bool IsPrimaryForSpec(Player* bot, ItemTemplate const* proto)
{
    ...
    const bool isJewelry = proto->InventoryType == INVTYPE_TRINKET || proto->InventoryType == INVTYPE_FINGER ||
