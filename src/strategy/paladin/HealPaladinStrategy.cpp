...
{
    return { new NextAction("judgement of light", ACTION_DEFAULT) };
}
...
void HealPaladinStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(
        new TriggerNode("seal", { new NextAction("seal of wisdom", ACTION_HIGH) }));
    triggers.push_back(new TriggerNode(
        "medium mana", { new NextAction("divine illumination", ACTION_HIGH + 2) }));
    triggers.push_back(
        new TriggerNode("low mana", { new NextAction("divine favor", ACTION_HIGH + 1) }));
    triggers.push_back(new TriggerNode(
    ...
    triggers.push_back(new TriggerNode(
        ...
        { new NextAction("sacred shield on main tank", ACTION_CRITICAL_HEAL + 6) }));
}
