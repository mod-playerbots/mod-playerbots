...
void AfflictionWarlockStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(new TriggerNode("unstable affliction on attacker", { new NextAction("unstable affliction on attacker", 19.0f) }));
    triggers.push_back(new TriggerNode("corruption", { new NextAction("corruption", 18.0f) }));
    triggers.push_back(new TriggerNode("unstable affliction", { new NextAction("unstable affliction", 17.5f) }));
    triggers.push_back(new TriggerNode("haunt", { new NextAction("haunt", 16.5f) }));

    // Drain Soul as execute if target is low HP // Shadow Trance for free casts
    triggers.push_back(new TriggerNode("shadow trance", { new NextAction("shadow bolt", 16.0f) }));
    triggers.push_back(new TriggerNode("target critical health", { new NextAction("drain soul", 15.5f) }));
