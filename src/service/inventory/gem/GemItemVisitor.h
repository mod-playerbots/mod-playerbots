...
class GemItemVisitor : public AbstractItemVisitor
{
public:
	bool process(Player* player, Item* item) override
	{
		...
		// @TODO: Implement logic on gem processing.
		return false;
	}
	...
	bool visit(Player* player, Item* item) override
	{
		...
		{
			LOG_ERROR("playerbots", "Item {} was not of class GEM ({}), found {}", itemTemplate->ItemId, std::to_string(ITEM_CLASS_ARMOR), std::to_string(itemTemplate->Class));
		...
		{
			LOG_ERROR("playerbots", "Item {} was a useful GEM item", itemTemplate->ItemId);
