...
class ConsumableConsumableItemVisitor final : public AbstractItemVisitor
{
public:
	...
	bool process(Player* player, Item* item) override
	{
		...
		const bool isRoguePoison = this->getRoguePoisonsItemsGUIDs().find(itemTemplate->ItemId) != this->getRoguePoisonsItemsGUIDs().end();

		if (playerClass != CLASS_ROGUE && isRoguePoison)
			return false;

		const bool isMageItem = this->getMageItemsGUIDs().find(itemTemplate->ItemId) != this->getMageItemsGUIDs().end();

		if (playerClass != CLASS_MAGE && isMageItem)
			return false;

		const bool isWarlockItem = this->getWarlockItemsGUIDs().find(itemTemplate->ItemId) != this->getWarlockItemsGUIDs().end();

		if (playerClass != CLASS_WARLOCK && isWarlockItem)
			return false;

		const bool isUnwantedItem = this->getUnwantedItemsGUIDs().find(itemTemplate->ItemId) != this->getUnwantedItemsGUIDs().end();

		if (isUnwantedItem)
			return false;

		const bool isQuestConsumableItem = this->getQuestConsumableItemsGUIDs().find(itemTemplate->ItemId) != this->getQuestConsumableItemsGUIDs().end();

		if (isQuestConsumableItem && !player->HasQuestForItem(itemTemplate->ItemId))
			return false;
...
protected:
	...
	const std::unordered_set<uint32_t> getUnwantedItemsGUIDs()
	{
		return {
			835, // Large Rope Net
			1127, // Flash Bundle
	...
	const std::unordered_set<uint32_t> getQuestConsumableItemsGUIDs()
	{
		return {
	...
	const std::unordered_set<uint32_t> getRoguePoisonsItemsGUIDs()
	{
		return {
			2892, // Deadly Poison
	...
	const std::unordered_set<uint32_t> getUsefulConsumableItemsGUIDs()
	{
		return {
	...
	const std::unordered_set<uint32_t> getWarlockItemsGUIDs()
	{
		return {
	...
	const std::unordered_set<uint32_t> getMageItemsGUIDs()
	{
		return {
			5513, // Mana Jade
			8007, // Mana Citrine
