...
class TradeGoodItemVisitor : public AbstractItemVisitor
{
public:
	...
	bool visit(Player* player, Item* item) override
	{
		...
		{
			LOG_ERROR("playerbots", "Item {} was not of class TRADE GOODS ({}), found {}", itemTemplate->ItemId, std::to_string(ITEM_CLASS_ARMOR), std::to_string(itemTemplate->Class));
		...
		{
			LOG_ERROR("playerbots", "Item {} was a useful TRADE GOODS item", itemTemplate->ItemId);
