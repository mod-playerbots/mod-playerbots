...
public:
    ...
    {
        // Only intercept whispers to playerbots
        ...
        // Hotfix: prevent crash when whispering "logout" to a bot
    ...
    {
        // Hotfix: prevent crash when saying "logout" in channel chat
        // https://github.com/mod-playerbots/mod-playerbots/pull/1838
        std::string lowered = msg;
        std::transform(lowered.begin(), lowered.end(), lowered.begin(),
                       [](unsigned char c) { return std::tolower(c); });
        ...
        PlayerbotMgr* const playerbotMgr =
            PlayerbotsMgr::instance().GetPlayerbotMgr(player);

        // Forward channel commands to owned bots for specific channel types
        if (playerbotMgr != nullptr && (channel->GetFlags() & 0x18))
        {
            playerbotMgr->HandleCommand(type, msg);
        }

        // Random bots always receive channel commands
